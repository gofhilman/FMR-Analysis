%fileNames = ["6_Py10nmvsTemp.txt"];
fileNames = ["01_FeO_sampleG_Bin_RT.txt"];
data = [];
for it_var = 1:length(fileNames)
    temp_data = importdata(fileNames(it_var));
    data = [data; temp_data(:, 3:6)];  % Select columns 2 to 5
end
%for it_var = 1:length(fileNames)
...
%    -0.9987   26.2000
        ...
        H_data = exp_data(freq_it_var:n_freq(exp_it_var):end, H_Col)./10; % Why
...
function [fit_Lorentz, fit_param] = fit_data_fcn(freq_data, H_data, S12_data, T_val, freq_it_var, exp_it_var, c, w)
    ...
    ft = fittype( 'w*A/(4*(x-center)^2+w^2)+offset-(w*B*(x-center))/(4*(x-center)^2+w^2)+k*x', 'independent', 'x', 'dependent', 'y' );
