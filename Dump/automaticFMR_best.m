...
mkdir(['Figures' filen(1:end-4) 'test'])
fileDir = strcat(pwd,['\Figures' filen(1:end-4) 'test']);
...
%     ini = 0;
    ...
    %---------------------------- Lorentizian Fitting ---------------------
    ft = fittype( 'A*w/(4*(x - center)^2+w^2) + offset  - (w*B*(x-center))/(4*(x - center)^2+w^2) + k*x', 'independent', 'x', 'dependent', 'y' );
    c = fcenter;                                
    a = ((max(s21)-min(s21))*w)/2;            
    off = max(s21); b = a*2;k = 1;                                      
    startpoints = [a b c k off 0.01 ];
    lower = [-1 -1 0   -5 -50 0.001];
    upper = [ 1  1 0.3  5 1 0.01];
