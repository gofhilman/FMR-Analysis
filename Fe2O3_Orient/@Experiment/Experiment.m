...
% ------------------------------------------------------------------------
        ...
        function crossP = getcrossingpoints(obj, filter)
            ...
            while true
                ...
                hold on
                plot(fieldVal, freqVal, 'o', 'Color', 'blue');
                hold off
            end
            obj.crossingPoints = crossP;
        end

        function trialanderror(obj, filter, dmF, eaF, gamma)
            ...
            y = gamma .* sqrt(field .* (field + dmF) + 2*eaF);
            plot(field, y, 'r', 'LineWidth', 2);
            hold off;
        end
        ...
        function makecrossingfit(obj, dfR)
            ...
            switch obj.magnetization.type 
                ...
                case 4
                    kittelEq = @(H) obj.kittelParameters.gyromagneticRatio.value ...
                               .* sqrt(H .* (H + obj.kittelParameters.dmField.value) + ...
                               2 * obj.kittelParameters.eaField.value);
                    deltaFreqFMR = @(H) obj.kittelParameters.gyromagneticRatio.value .* ...
                                   sqrt(1 + (obj.kittelParameters.gyromagneticRatio.value .* obj.kittelParameters.dmField.value ./ ...
                                        (4 .* pi .* kittelEq(H))).^2) .* damping(H);
            end
        ...
        function kittelParams = makeinplanekittelfit(obj, startEffMag, ...
            ...
            dmField = PhysicalMagnitude;
            eaField = PhysicalMagnitude;
        ...
        function kittelParams = makeafmkittelfit(obj, startDMField, ...
                                                     startEAField, ...
                                                     startGyroR)
            ...
            frequency = obj.resonanceParameters(:,Column.frequency);
            if nargin < 3
                startPoint = [-0.01*sign(field(1)), 0, 28];
            else
                startPoint = [startDMField, startEAField, startGyroR];
            end
            

            % Calculate fit parameters for Kittel equation
            [fitresult, ~] = fitafmkittel(field, ...
                                                frequency, ...
                                                startPoint, ...
                                                obj.fieldUnits, ...
                                                obj.frequencyUnits);
            ...
            effectiveMagnetization = PhysicalMagnitude;
            magneticSusceptibility = PhysicalMagnitude;
            dmField = PhysicalMagnitude(fitresult.dmF, ...
                                        dmConfInt, ...
                                        obj.fieldUnits);
            eaField = PhysicalMagnitude(fitresult.eaF, ...
                                        eaConfInt, ...
                                        obj.fieldUnits);
        ...
        function dampingParams = makedampingfit(obj)
            ...
            % Create physical magnitude instances to store data
            damping = PhysicalMagnitude(gamma * fitresult.p1 / 2, ...
                                        gamma * dampingConfInt / 2, ...
                                        Scale.one);
...
function [fitresult, gof] = fitlorentziantodata(xData, yData, startPoint)
    ...
    ft = fittype(['fwhm * A / (4 * (x-hr)^2 + fwhm^2) -' ... 
...
function [fitresult, gof] = fitafmkittel(xData, yData, startPoint, ...
                                             fieldU, freqU)  
